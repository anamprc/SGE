CLASS zbp_generate_bookings_ama DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_ama IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_ama.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_ama.
*   insert the new table entries
    INSERT ztbooking_ama FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_ama INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
